...
import { convertCucumberFeatures, readCucumberReport } from "./cucumber-result-conversion";
import { uploadCucumberResults } from "./cucumber-result-upload";
import { convertCypressResults } from "./cypress-result-conversion";
...
export async function cypressXrayPlugin(parameters: RuntimeParameters) {
    ...
    if (containsCypressTests) {
        try {
            const xrayJson = convertCypressResults({
                context: {
    ...
    if (containsCucumberTests) {
        try {
            const reportData = await readCucumberReport({
                cypress: {
            ...
            const cucumberJson = convertCucumberFeatures({
            ...
            const uploadResult = await uploadCucumberResults({
                client: parameters.clients.xray,
                cucumberJson: cucumberJson.features,
                multipartInfo: multipartInfoData.multipartInfo,
            });
