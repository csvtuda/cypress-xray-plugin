...
void describe(relative(cwd(), __filename), () => {
    void describe(getIssueSnapshots.name, () => {
        void it("returns no snapshot data if no issues are specified", async (context) => {
            ...
            const result = await getIssueSnapshots({
    ...
    void describe(restoreIssueSnapshots.name, () => {
        void it("handles unrecoverable issues", async (context) => {
            const editIssueMock = context.mock.fn<HasEditIssueEndpoint["editIssue"]>();
            const configureMock = context.mock.fn<Logger["configure"]>();
            const logErrorToFileMock = context.mock.fn<Logger["logErrorToFile"]>();
            const logToFileMock = context.mock.fn<Logger["logToFile"]>();
            const messageMock = context.mock.fn<Logger["message"]>();
            await restoreIssueSnapshots({
                client: { editIssue: editIssueMock },
                logger: {
                    configure: configureMock,
                    logErrorToFile: logErrorToFileMock,
